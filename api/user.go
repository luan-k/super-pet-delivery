...
func (server *Server) loginUser(ctx *gin.Context) {
	...
	ctx.SetCookie("access_token", accessToken, int(server.config.AccessTokenDuration.Seconds()), "/", "54.94.135.147", false, false)
	// Set the refresh token as an HTTP-only cookie
	ctx.SetCookie("refresh_token", refreshToken, int(server.config.RefreshTokenDuration.Seconds()), "/", "54.94.135.147", false, true)
