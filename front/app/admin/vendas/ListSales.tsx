...
const ListSales: React.FC<ListSalesProps> = ({ className }) => {
  ...
  const handleButtonClick = async (TypeOfPdf: string): Promise<void> => {
    ...
    try {
      ...
      const response = await fetch("http://15.228.47.144:8080/pdf/", {
        method: "POST",
        headers: {
  ...
  const fetchSales = async (
    ...
    try {
      const token = Cookies.get("access_token");
      let url = `http://15.228.47.144:8080/sales?page_id=${pageId}&page_size=${pageSize}`;
    ...
    try {
      ...
      const promises = clientIds.map(async (clientId) => {
        const response = await fetch(
          `http://15.228.47.144:8080/clients/${clientId}`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleCheckAll = async () => {
    ...
    if (allChecked) {
      ...
      const response = await fetch("http://15.228.47.144:8080/sales/all", {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
